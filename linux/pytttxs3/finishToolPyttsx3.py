...
# pip install pyperclip keyboard mouse PyQt5 mss numpy pytesseract pillow pyttsx3 opencv-contrib-python
import ctypes
import sys
...
def set_cursor(cursor_file):
    os.environ["XCURSOR_PATH"] = os.path.dirname(cursor_file)
    os.environ["XCURSOR_THEME"] = os.path.splitext(os.path.basename(cursor_file))[0]
    subprocess.run(
        ["xrdb", "-merge"],
        input=f"Xcursor.theme: {os.environ['XCURSOR_THEME']}\nXcursor.size: 24\n",
        text=True,
    )
    subprocess.run(["xsetroot", "-cursor_name", os.environ["XCURSOR_THEME"]])
